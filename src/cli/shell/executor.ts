...
export interface ExecResult {
  stdout: string;
  stderr: string;
  /** false if command failed to run or exited with non-zero code */
  success: boolean;
}
...
export async function executeCommand(
  ...
  if (platform === 'win32') {
    ...
    // On Unix, use explicit shell so spawn path is predictable (avoids ENOENT in some envs)
...
export async function executeCommandWithOutput(
  ...
  if (!result.success && result.stderr && !options.silent) {
    console.error(result.stderr);
  }

  if (result.success) {
    // For npm install, skip verbose stdout
    if (!command.includes('npm install') && result.stdout && !options.silent) {
      console.log(result.stdout);
    }
    if (!command.includes('npm install') && result.stderr && !options.silent) {
      console.error(result.stderr);
    }
  } else if (command.includes('npm install') && result.stderr && !options.silent) {
    const errorLines = result.stderr
      .split('\n')
      .filter((line) => line.includes('error') || line.includes('Error') || line.includes('ERR!'));
