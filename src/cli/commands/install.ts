...
export async function installHooks(options: InstallOptions): Promise<void> {
  ...
  try {
    ...
    const existingHooksPaths = await findExistingPaths(hooksPaths, hooksDir);
    const existingRulesPaths = await findExistingPaths(rulesPaths, rulesDir);
    const existing = [...existingHooksPaths, ...existingRulesPaths];

    if (existing.length > 0) {
      console.log('⚠️  The following paths already exist and would be overwritten:\n');
      for (const p of existing) {
        console.log(`   ${p}`);
      }
      console.log('');
      const overwrite = await confirmAction(
        'Overwrite these paths? (backup or rename them first if you need to keep current content)',
        false
      );
      if (!overwrite) {
    ...
    if (mergeResult.skipped > 0) {
      console.log(`⚠️  Skipped ${mergeResult.skipped} hook(s) that already exist:`);
