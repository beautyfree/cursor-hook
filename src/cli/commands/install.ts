...
import { downloadRepository, loadConfigFromRepo, cleanupTempDir, isTempDir } from '../git/downloader';
...
export async function installHooks(options: InstallOptions): Promise<void> {
  ...
  try {
    ...
    const mergeResult = mergeHooks(existingHooks, config, hooksJsonPath, hooksDir, isProjectInstall);
